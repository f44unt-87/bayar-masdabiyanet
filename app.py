...
def get_cleaned_data(ttl_time="1m"):
    """Membaca data dan memaksa No HP menjadi teks bersih."""
    try:
        df = conn.read(ttl=ttl_time)
        # Salinan agar tidak merubah data asli saat proses cleaning di memori
        df_clean = df.copy()
        
        if 'No HP' in df_clean.columns:
            # Ubah ke string, hapus .0 (jika terbaca float), dan hapus 'nan'
            df_clean['No HP'] = df_clean['No HP'].astype(str).replace(r'\.0$', '', regex=True).replace('nan', '')
...
def buat_link_wa(nama, tagihan, tgl, no_hp):
    """Membuat link WA yang kebal terhadap error tipe data."""
    if not no_hp or str(no_hp).lower() == 'nan' or no_hp == "":
        return None
    
    # Ambil angka saja (membuang spasi, strip, atau titik desimal)
    num = "".join(filter(str.isdigit, str(no_hp).split('.')[0]))
    
    if not num: return None
    
    # Standarisasi ke format internasional (62)
    if num.startswith('0'):
        num = '62' + num[1:]
...
# Lookup No HP Otomatis
no_hp_terdeteksi = ""
if nama_aktif:
    df_lookup = get_cleaned_data()
    if not df_lookup.empty:
        match = df_lookup[df_lookup['Nama'].astype(str).str.lower() == nama_aktif.lower()]
        if not match.empty:
...
if submit_button:
    if not nama_aktif:
        ...
        with st.spinner("Menyimpan ke Google Sheets..."):
            try:
                # Ambil data asli untuk diupdate
                df = conn.read(ttl="0")
                ...
                mask = df['Nama'].astype(str).str.lower() == nama_aktif.lower()
